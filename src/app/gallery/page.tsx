    ...
    <div className="pt-16">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-16 pb-12">
        ...
        <p className="text-sm text-muted-foreground max-w-lg leading-relaxed">
          ภาพถ่ายความละเอียดสูงจากทุกนัดแข่งขัน บันทึกโดยช่างภาพกีฬมืออาชีพ
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24">
        {albums.length === 0 ? (
          <div className="py-20 border border-dashed border-border/60 rounded-sm text-center">
            <p className="text-sm text-muted-foreground">ยังไม่มีแกลเลอรี่ให้แสดงในขณะนี้</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {albums.map((album) => (
              ...
              >
                ...
                <p className="text-[12px] text-muted-foreground">{album.date} · {album.photos?.length || 0} รูป</p>
              </Link>
            ))}
          </div>
        )}
